...
public class BaremetalKickStartServiceImpl extends BareMetalPxeServiceBase implements BaremetalPxeService {
    ...
    private List<String> parseKickstartUrl(VirtualMachineProfile profile) {  // add new method for parsing for wds based url
    ...
    private String parseTemplateUrlForWindows(VirtualMachineProfile profile) {  //Think of merging this method with Linux counter part or make it generic with any type - Linux/XS/Windows..
        String tpl = profile.getTemplate().getUrl();
        assert tpl != null : "How can a null template get here!!!";
        String[] tpls = tpl.split(";");
        CloudRuntimeException err =
                new CloudRuntimeException(
                        String.format(
                                "template url[%s] is not correctly encoded. it must be in format of WdsClientUnattend=path-to-client-unattend-file;BootImagePath=path-of-boot-image-in-wds",
                                tpl));
    ...
    private boolean preparePxeInBasicZone(VirtualMachineProfile profile, NicProfile nic, DeployDestination dest, ReservationContext context) throws AgentUnavailableException, OperationTimedoutException {
        ...
        if (pxeVo == null) {
            //return true; //TODO remove it once it is merged with dhcp/dns offload. //this is hack to bypass kickstart
        ...
        //if else here to call different command object in case of windows
        VMTemplateVO template = _tmpDao.findById(profile.getTemplateId());
        boolean isWindows = _guestOsCategoryDao.findById(_guestOsDao.findById(template.getGuestOSId()).getCategoryId()).getName().equalsIgnoreCase("Windows"); //TODO - remove this logic on some utility class - as it is used at many places.
        PrepareKickstartPxeServerCommand cmd = new PrepareKickstartPxeServerCommand();
        if(isWindows) { //TODO - check only if customer need pre-staging capacity through Management Server otherwise default setting in WDS would work.
            cmd.setWindows(true);
            String wdsArgs = parseTemplateUrlForWindows(profile);
            cmd.setAdditionalParams(wdsArgs);
        } else {
    ...
    public BaremetalPxeVO addPxeServer(AddBaremetalPxeCmd cmd) {
        ...
        try {
            resource.configure("KickStart PXE resource", params); //Need to distinguish if it is for windows pxe. -- possibly by tftp null or do some changes in UI. Pass the same in params.
        } catch (Exception e) {
